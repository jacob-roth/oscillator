...
function U!(C::Cache)
    ...
    @simd for i in eachindex(C.S.q)
        @simd for j in eachindex(C.S.q)
            @inbounds @views C.E.ke += C.B[i,j] * log(abs(C.S.q[i])) * log(abs(C.S.q[j]))
        end
    end
    C.E.ke *= 0.5
...
function dUdq!(C::Cache)
    ## B log(q) ∘ 1/q
    mul!(C.grad_q, C.B, log.(abs.(C.S.q)))
    @simd for i in eachindex(C.S.q)
        @inbounds @views C.grad_q[i] /= abs(C.S.q[i]) * sign(C.S.q[i])
    end
    nothing
end
...
function U(q, p, B::AbstractArray)
    return 0.5 * (p'*p + (log.(abs.(q))' * B * log.(abs.(q))))
end
